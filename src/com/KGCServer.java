	...
	public static boolean getParams() {
		...
		try {
			...
			if (error_no.equals("0")) { // 成功
				...
				// 保存公开参数到文件中
				CommonFileManager.writeObjectToFile(params, UserInfo.paramsPath + CommonDef.paramsAffix);
			} else {
				res = false;
	...
	public static boolean getPartKey(String name) {
		...
		try {
			...
			if (error_no.equals("0")) { // 成功
				...
				// 保存部分私钥到文件中
				CommonFileManager.saveBytesToFilepath(partKey, UserInfo.paramsPath + CommonDef.partKeyAffix(name));
			} else {
				res = false;
				JOptionPane.showMessageDialog(null, error_info, "错误", JOptionPane.ERROR_MESSAGE);
			}
